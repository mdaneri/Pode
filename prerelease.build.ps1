...
Add-BuildTask ProcessPRs UpdateDevelop, Create-NewBranch , Commit-VersionJson, {
    ...
    $mainPr2 = @((gh pr view 1543  --repo Badgerati/Pode --json 'number,title,url,mergeStateStatus' | ConvertFrom-Json))
    ...
    foreach ($pr in $prs) {
            ...
            if ($mergeExitCode -ne 0) {
                do {
                    ...
                    if ($choice -eq 'a') {
                        ...
                        # Call the Group-LanguageResource function
                        Group-LanguageResource
        ...
        if ($SkipTest -notcontains $pr.number) {
            do {
        ...
        do {
            ...
            if ($commitExitCode -ne 0) {
                Write-Output "Commit failed for PR #$($pr.number). (R)etry/(Q)uit/(C)ontinue?"
                $choice = Read-Host
                if ($choice -eq 'q') { exit 1 }
